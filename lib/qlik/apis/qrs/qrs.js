...
function createTo(node, parent) {
    var retVal = [];
    Object.keys(node).forEach(function(key) {
        var str = "";
        if(key.match(/{([^{]+)}/g)) {
            str += "/**" + "\n";
            if(typeof parent === 'undefined') str += " * {@link %parentDoc%|See parent documentation}"  + "\n";
            str += " * @namespace" + "\n";
            if(typeof parent !== 'undefined') str += " * @memberof " + parent + "\n";
            str += " */" + "\n";
            str += key.replace(/{([^{]+)}/g, '$1') + ": function(" + key.replace(/{([^{]+)}/g, '$1') + ") {" + "\n";
            str += "return {" + "\n";
            str += createTo(node[key], ((typeof parent !== 'undefined') ? parent + "." : "") + key.replace(/{([^{]+)}/g, '$1')).map(function(item) { return item.str; }).join(",\n") + "\n";
            str += "};\n";
            str += "}"
        }
        else {
            if(typeof node[key].function !== 'undefined') {
                ...
                str += " */" + "\n";
                str += key + ": {" + "\n";
                str += createTo(node[key], ((typeof parent !== 'undefined') ? parent + "." : "") + key).map(function(item) { return item.str; }).join(",\n") + "\n";
                str += "}";
            }
        }
        retVal.push({ key: key, str: str});
    });
    return retVal;
}
...
function parseAPI(apiString) {
    ...
    method.comment = "/**" + "\n" +
        ...
        " * @example" + "\n" +
        " * ```javascript" + "\n" +
        " * qrsApi." + method.key.map(function(item) { return item.replace(/{([^{]+)}/g, '$1($1)'); }).join(".") + "(" + defaultParams.concat(result.queryString).map(function(item) { return item.value; }).join(', ') + ").then(function(" + ((result.api.extended.returntype && result.api.extended.returntype !== "Void") ? result.api.extended.returntype : "") + ") {" + "\n" +
