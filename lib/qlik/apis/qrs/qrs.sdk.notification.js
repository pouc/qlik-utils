    ...
    return {
        ...
        notification: {
            ...
            post: function(postParams, typename, id, filter, condition, changetype, propertyname) {
                var restUri = url.parse(options.restUri);
                return base.request({
                    restUri: 'https://' + restUri.hostname + ':' + restUri.port + '/qrs/notification' + (core.ifNotUndef(typename, '&name=' + typename, '') +
            ...
            changes: {
                ...
                get: function(since, types) {
                    var restUri = url.parse(options.restUri);
                    return base.request({
                        restUri: 'https://' + restUri.hostname + ':' + restUri.port + '/qrs/notification/changes' + (core.ifNotUndef(since, '&since=' + since, '') +
