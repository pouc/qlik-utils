        ...
        replacesDef = replacesDef.map(function(item) {
            return item.replaces.map(function(replace) {
                return replace.value.map(function(val) {
                    return {marker: replace.marker, value: val};
                });
        ...
        return Q().then(function() {
            ...
            replacesDef.forEach(function(replaceDef) {
                ...
                if (replaceDef.targetApp)
                    replaceDef.targetApp = sprintf(replaceDef.targetApp, {templateAppName: replaceDic[arrayTemplateFound[0].qDocId], replaceValue: replaceValue, replace: replaceDef.replace});

                if (replaceDef.replaceApp)
                    replaceDef.replaceApp = sprintf(replaceDef.replaceApp, {templateAppName: replaceDic[arrayTemplateFound[0].qDocId], replaceValue: replaceValue, replace: replaceDef.replace, targetAppName: replaceDef.targetApp});

                if (replaceDef.createReloadTask && replaceDef.createReloadTask.task && replaceDef.createReloadTask.task.name)
                    replaceDef.createReloadTask.task.name = sprintf(replaceDef.createReloadTask.task.name, {templateAppName: replaceDic[arrayTemplateFound[0].qDocId], replaceValue: replaceValue, replace: replaceDef.replace, targetAppName: replaceDef.targetApp});
                    ...
                    replaceDef.customProperties.forEach(function(customProperty) {
                        if (customProperty.values) {
                            customProperty.values = customProperty.values.map(function(value) {
                                return sprintf(value, {templateAppName: replaceDic[arrayTemplateFound[0].qDocId], replaceValue: replaceValue, replace: replaceDef.replace, targetAppName: replaceDef.targetApp});
                            });
                        }
                    });
