...
exports.Advanced = {
    ...
    openWebSocket: function(options, params, task) {
        ...
        return step.then(function(ticket) {
            ...
            if (typeof options.cookie !== 'undefined' && typeof sockets[options.cookie] !== 'undefined' && typeof sockets[options.cookie][appname || 'default'] !== 'undefined') {
                ...
                if (task) task.running('error', 'Impossible to open a new ws because of the following error: "%s"', err.message);
    ...
    dynamicAppClone: function(options, params, task) {
        ...
        var defaultTargetApp = '%(templateAppName)s %(replace.value)s';
        ...
        var createReloadTask = core.ifNotUndef(params.createReloadTask, defaultCreateReloadTask);
        ...
        replacesDef = replacesDef.map(function(item) {
            return item.replaces.map(function(replace) {
                return replace.value.map(function(val) {
                    return {
                        replace: {marker: replace.marker, value: val},
            ...
            replacesDef.forEach(function(replaceDef) {
                ...
                replacesDef.forEach(function(replaceDef) {
                    ...
                    if (replaceDef.createReloadTask && replaceDef.createReloadTask.task && replaceDef.createReloadTask.task.name)
                        replaceDef.createReloadTask.task.name = sprintf(replaceDef.createReloadTask.task.name, {templateAppName: replaceDic[arrayTemplateFound[0].qDocId], replace: replaceDef.replace, targetAppName: replaceDef.targetApp});
                        ...
                        replaceDef.customProperties.forEach(function(customProperty) {
                            if (customProperty.values) {
                                customProperty.values = customProperty.values.map(function(value) {
                                    return sprintf(value, {templateAppName: replaceDic[arrayTemplateFound[0].qDocId], replace: replaceDef.replace, targetAppName: replaceDef.targetApp});
                                });
                            }
                        });
                });
    ...
    loopAndReduce: function(options, params, task) {
        ...
        var loopReduceApp = params.reduce.app;
        var maxParDup = core.ifNotUndef(params.reduce.maxParDup, 5);
        var overwriteApp = core.ifNotUndef(params.reduce.overwriteApp, false);
        var keepApp = core.ifNotUndef(params.reduce.keepApp, true);
        var createReloadTask = core.ifNotUndef(params.reduce.createReloadTask, false);
        ...
        return exports.Advanced.export(options, exportParams, task).then(function(reply) {
            ...
            var dynamicCloneParams = {
                templateApp: loopReduceApp,
                maxParDup: maxParDup,
                replacesDef: reply.map(function(item) {
                    return {
                        replaces: {marker: '%Replace me!%', value: core.ifNotUndef(reduceColumn, item[1], undefined)},
                ...
                keepApp: keepApp,
                createReloadTask: createReloadTask
            };

            return exports.Advanced.dynamicAppClone(extend(true, {}, options, {retry: 1}), dynamicCloneParams, task);
        });
